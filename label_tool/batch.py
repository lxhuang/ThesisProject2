...
class Batch:
	...
	video_set = set()
	outlier_buffer    = dict()
	coder_per_video   = dict() # we run another 2 batches
	...
	def _aggregate(self, ts_set, video_len):
		...
		for ts in ts_set:
			for t in ts:
				beg = (t-window) if (t-window) > 0 else 0
				end = (t+window) if (t+window) < video_len else video_len
	...
	def _getDataOfVideo(self, videoId, aggregated):
		...
		try:
			...
			for c in cls.coder_set:
				ts = []
				if c not in cls.coder_per_video[videoId]:
					continue
	...
	def process(self, videoId, coders):
		...
		for coder in coders:
			if coder not in cls.coder_per_video[videoId]:
				continue
	...
	def observe(self, videoId, attr, num):
		try:
			...
			for coder in cls.coder_info_buffer.iterkeys():
				if (coder not in cls.coder_per_video[videoId]) or (coder not in cls.coder_set):
					continue
